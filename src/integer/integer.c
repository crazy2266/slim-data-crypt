...
static sdc_word_t modinv_word(sdc_word_t a, sdc_word_t m) {
    ...
    /* At termination (v == gcd == 1 assumed), x2 holds a*x2 â‰?v(final) ... 
 ...
 *
 * We want d such that d * e â‰?1 (mod Ï†).
 * This means d * e = k * Ï† + 1 for some integer k.
 * Rearranging: d = (k * Ï† + 1) / e.
 *
 * For this to yield an integer, we need:
 *     k * Ï† â‰?-1 (mod e)
 *     k â‰?-Ï†^{-1} (mod e)
 *
 * Since e is a small public exponent (typically 65537), Ï†^{-1} mod e
