 ...
 * 
 * ChaCha20 Raw - 5-block-per-call keystream generation:
...
#if SDC_64BIT  // ARM64, ARMv8 neon is always available
...
void sdc_chacha20raw_init(sdc_chacha20_ctx *ctx, const uint8_t key[32],
    ...
    uint32_t nonce_lo = load32_le(nonce);
    uint32_t nonce_hi = load32_le(nonce + 4);
    ...
    for (int i = 0; i < 8; i++) {
        ctx->state1[i + 4] = vdupq_n_u32(key_w[i]);
    }
    ...
    ctx->state1[14] = vdupq_n_u32(nonce_lo);
    ctx->state1[15] = vdupq_n_u32(nonce_hi);
    ...
    for (int i = 0; i < 8; i++) {
        ctx->state2[i + 4] = key_w[i];
    }
    ctx->state2[12] = (uint32_t)(counter + 4);
    ctx->state2[13] = (uint32_t)((counter + 4) >> 32);
    ctx->state2[14] = nonce_lo;
    ctx->state2[15] = nonce_hi;
    ctx->buf_used = 320; /* forces next_block() on first crypt() call */
}
...
void sdc_chacha20raw_init(sdc_chacha20_ctx *ctx, const uint8_t key[32],
                          const uint8_t nonce[8], uint64_t counter) {
    if (!ctx || !key || !nonce) return;
    uint32_t *in = ctx->state;
    in[0] = 0x61707865;
    in[1] = 0x3320646e;
    in[2] = 0x79622d32;
    in[3] = 0x6b206574;
    for (int i = 0; i < 8; i++) {
        in[i + 4] = load32_le(key + 4 * i);
    }
    in[12] = (uint32_t)counter;
    in[13] = (uint32_t)(counter >> 32);
    in[14] = load32_le(nonce);
    in[15] = load32_le(nonce + 4);
    ctx->buf_used = 64;
}
