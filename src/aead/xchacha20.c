...
static void hchacha20(uint32_t out[8], const uint32_t in[16]) {
    uint32_t x[16];
    memcpy(x, in, sizeof(uint32_t) * 16);
    for (int i = 0; i < 10; i++) {
        QR(x[0], x[4], x[8],  x[12]);
...
void sdc_xchacha20_init(sdc_chacha20_ctx *ctx, const uint8_t key[32],
    ...
    in[12] = load32_le(nonce);
    in[13] = load32_le(nonce + 4);
    in[14] = load32_le(nonce + 8);
    in[15] = load32_le(nonce + 12);
    hchacha20(hkey, in);
    ...
    for (int i = 0; i < 8; i++) {
        ctx->state[4 + i] = hkey[i];
    }
    ctx->state[12] = (uint32_t)counter;
    ctx->state[13] = (uint32_t)(counter >> 32);
    ctx->state[14] = load32_le(nonce + 16);
    ctx->state[15] = load32_le(nonce + 20);
    ctx->buf_used = 64;
}
